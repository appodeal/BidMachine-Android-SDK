...
public class DefaultMediaPicker implements VASTMediaPicker {
	...
	private boolean isVPAIDMediaFileCompatible(VASTMediaFile media) {
		return media.getType().equals("application/javascript") && media.getApiFramework().equals("VPAID");
	}
	...
	private VASTMediaFile getBestMatch(List<VASTMediaFile> list) {
		...
		//Check if it is application/javascript VPAID
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
			for (VASTMediaFile media : list) {
				if (isVPAIDMediaFileCompatible(media)) {
					return media;
				}
			}
		}
